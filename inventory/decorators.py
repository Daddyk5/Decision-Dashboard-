...
def user_has_role(user, role):
    group_names = ROLE_GROUP_ALIASES.get(role, (ROLE_GROUPS[role],))
...
def role_required(*roles):
    def decorator(view_func):
        ...
        def wrapped(request, *args, **kwargs):
            if request.user.is_superuser or request.user.groups.filter(name=ROLE_GROUPS['super_admin']).exists():
...
def current_entity_role(user):
    if user.is_superuser or user.groups.filter(name=ROLE_GROUPS['super_admin']).exists():
